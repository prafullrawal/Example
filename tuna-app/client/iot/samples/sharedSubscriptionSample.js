...
pg.connect(pgConString, function(err, client) {
    ...
    client.query("INSERT INTO foo(shipmentid,lasttemp,name) values("+response.recordId+","+json.temperature+",'Waste')"
     ...
     ,(err,res)=>{
       ...
       client.end();
 							...
 							for(var i=0; i < response.arrayKeys.length; i++){
									...
									var txid =  await ChangeStateTowaste(key,recordStatus,channelName);		
									console.log(txid);				
...
pg.connect(pgConString, function(err, client) {
    ...
    client.query("INSERT INTO foo(shipmentid,lasttemp,name) values("+req+",'Less then 17','Fresh')"
...
function  ChangeStateTowaste(key,recordStatus,channelName,res){
