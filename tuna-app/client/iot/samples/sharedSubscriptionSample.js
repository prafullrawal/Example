  ...
  start_reading_temp:function(req,res){
	...
	app.use(session({secret: 'ssshhhhh',saveUninitialized: true,resave: true}));
        ...
        //  console.log("Device Event from :: "+deviceType+" : "+deviceId+" of event "+eventType+" with payload : "+payload);
          ...
          if(json.temperature > 16){
			session.res.recordId = json.recordId;
			session.res.temp.push(json.temperature);
			session.res.state = "Waste";
			console.log("Rejected : temp =" + JSON.stringify(session.res));
			
			// changing the state of item to waste
		
			var key = session.res.recordId;
			var recordStatus = session.res.state;
			...
			Fabric_Client.newDefaultKeyValueStore({ path: store_path
				...
				// updateRecordStatus - requires 2 args , ex: args: ['1', 'Barry'],
		...
		//console.log("this is we want :"+tx_id.getTransactionID());
		}
		  else{
            if(session.status!="Waste"){
              session.res.recordId = json.recordId;
              session.res.temp.push(json.temperature)
              session.res.state = "Fresh";
              console.log("Accepted : temp =" + JSON.stringify(session.res));
            }	
          }
	});
},

	disconnectIOT:function(req,res){
	...
	get_final_status:function(req,res){
		...
		res = JSON.stringify(session.res);
		console.log(res);     
		session.res.state = '';	
		return res;
	}
